...
def _capture() -> dict:
    return {
        ...
        "macro": {
            "us_10y_yield": {"latest": 4.5},
            "broad_usd_index": {"latest": 120.0},
            "fed_total_assets": {"latest": 6_590_000.0},
            "overnight_reverse_repo": {"latest": 2.125},
        },
        ...
        "coverage": {"source_status": {
            ...
            "fred_fed_total_assets": {"status": "LIVE"},
            "fred_overnight_reverse_repo": {"status": "LIVE"},
            "etf_flows": {"status": "LIVE"},
        }},
    }


def test_validate_capture_requires_live_etf_and_all_macro_series() -> None:
    ok, reason = validate_capture(_capture())
    assert ok is True
    assert reason == "ok"


def test_validate_capture_rejects_partial_macro_coverage() -> None:
    capture = _capture()
    capture["coverage"]["source_status"]["fred_overnight_reverse_repo"] = {"status": "ERROR"}
    ok, reason = validate_capture(capture)
    assert ok is False
    assert reason == "incomplete_macro_coverage"
