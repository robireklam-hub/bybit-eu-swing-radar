...
FEATURES = (
    "flow_book_concordance_60s",
    "side_microprice_displacement_bps_15s",
    "side_book_pressure_ratio_60s",
    "spread_bps_mean_15s",
)


def _rows(counts: tuple[int, int, int]) -> list[dict]:
...
def test_selects_earliest_ready_prefix_without_outcomes() -> None:
    rows = _rows((30, 15, 25))
    cohort, gate = effect_test_v3.select_earliest_ready_cohort(rows, SYMBOLS)
    ...
    assert cohort[-1]["signal_id"] == 60


def test_strategy_contamination_fails_closed() -> None:
    rows = _rows((20, 20, 20))
...
@pytest.mark.asyncio
async def test_outcome_query_is_forbidden_before_sample_gate(monkeypatch) -> None:
    rows = _rows((19, 20, 20))
    called = False
    ...
    async def _connect(*args, **kwargs):
        nonlocal called
        called = True
        raise AssertionError("DB connection must not be attempted")
    ...
    with pytest.raises(RuntimeError, match="below preregistered minimum"):
        await effect_test_v3.load_closed_outcomes("postgres://example", rows)
    assert called is False


def test_analysis_is_deterministic_descriptive_and_never_promotes() -> None:
    ...
    assert all(set(item["symbol_stratified"]) == set(SYMBOLS) for item in first["results"])


def test_missing_closed_outcome_waits_without_partial_effects() -> None:
    cohort = _rows((20, 20, 20))
    outcomes = [
        {"signal_id": row["signal_id"], "net_r": 0.1}
        for row in cohort[:-1]
    ]
