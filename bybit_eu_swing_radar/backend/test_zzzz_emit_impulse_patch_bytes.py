"""Temporary CI byte emitter. Intentionally fails after emitting gzip+base64."""
...
def test_emit_exact_impulse_patch_bytes() -> None:
    if os.getenv("GITHUB_ACTIONS") != "true" or os.getenv("GITHUB_HEAD_REF") != "fix/btc-impulse-breakout-coverage":
        return
    ...
    test_text = r'''from datetime import datetime, timezone
...
'''
    ...
    day_b64 = base64.b64encode(gzip.compress(text.encode(), compresslevel=9)).decode()
    test_b64 = base64.b64encode(gzip.compress(test_text.encode(), compresslevel=9)).decode()
    print("DAY_WORKER_GZIP_B64=" + day_b64)
    print("REGRESSION_TEST_GZIP_B64=" + test_b64)
    raise AssertionError("BYTE_EXPORT_COMPLETE")
