...
def test_backtest_writer_and_default_job_are_v074():
    text = (BACKEND / "backtest.py").read_text(encoding="utf-8")
    assert 'STRATEGY_VERSION = "0.7.4"' in text
    assert 'v074-90d-netrr-structural-barrier' in text
...
def test_version_isolation_keeps_flow_feature_0722():
    ...
    assert 'does NOT change the v0.7.4 STRICT gates' in flow
