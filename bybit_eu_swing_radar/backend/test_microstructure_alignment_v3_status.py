...
from app.research_microstructure_alignment_v3_api import build_alignment_v3_status
...
from scripts.production_microstructure_alignment_status_v3 import validate_alignment_status
...
def _production_payload(counts=None):
    ...
    return build_alignment_v3_status(
...
def test_v3_route_is_attached_by_production_research_composition():
    app = FastAPI()
...
def test_v3_status_is_v075_label_blind_and_closed_to_promotion():
    ...
    assert payload["spec"]["spec_version"] == "microstructure-forward-alignment-v3"
    assert payload["spec"]["parent_spec_version"] == "microstructure-forward-alignment-v2"
    assert payload["spec"]["preregistered_strategy_version"] == "0.7.5"
    assert payload["preregistered_strategy_version"] == "0.7.5"
    ...
    assert payload["spec"]["cohort_start_at"] == alignment_v3.COHORT_START_AT.isoformat()


def test_v3_coverage_failure_closes_effect_gate():
    payload = build_alignment_v3_status(
        {"ready_for_forward_feature_analysis": True},
...
def test_v3_waiting_state_is_explicit_when_no_forward_signals_exist():
    payload = build_alignment_v3_status(
        {"ready_for_forward_feature_analysis": True},
        [],
        SYMBOLS,
...
def test_v3_production_validator_accepts_only_frozen_contract():
    payload = _production_payload()
    assert validate_alignment_status(payload) == (True, "ok")


def test_v3_production_validator_fails_closed_if_any_research_gate_opens():
