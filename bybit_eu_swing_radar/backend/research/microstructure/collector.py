"""Research-only Bybit EU spot microstructure collector.

Maintains an in-memory L50 book and persists bounded 5-second feature buckets.
It is deliberately isolated from live strategy/scoring/execution.
"""
...
ON CONFLICT (symbol,bucket_start,bucket_seconds) DO UPDATE SET
    ...
    bid_depth_5_quote=EXCLUDED.bid_depth_5_quote,ask_depth_5_quote=EXCLUDED.ask_depth_5_quote,
    bid_depth_10_quote=EXCLUDED.bid_depth_10_quote,ask_depth_10_quote=EXCLUDED.ask_depth_10_quote,
    bid_depth_50_quote=EXCLUDED.bid_depth_50_quote,ask_depth_50_quote=EXCLUDED.ask_depth_50_quote,
    imbalance_5=EXCLUDED.imbalance_5,imbalance_10=EXCLUDED.imbalance_10,imbalance_50=EXCLUDED.imbalance_50,
...
def _bucket_start_ms(timestamp_ms: int, bucket_seconds: int) -> int:
    ...
    return timestamp_ms - timestamp_ms % width_ms
...
class MicrostructureConfig:
    ...
    def from_env(cls) -> "MicrostructureConfig":
        symbols = tuple(dict.fromkeys(
            item.strip().upper()
            for item in os.getenv("MICROSTRUCTURE_SYMBOLS", ",".join(DEFAULT_SYMBOLS)).split(",")
            if item.strip()
        ))
        config = cls(
            enabled=_truthy(os.getenv("MICROSTRUCTURE_RECORDER_ENABLED"), True),
    ...
    def validate(self) -> None:
        ...
        if len(self.symbols) > 12:
            raise ValueError("microstructure v1 is capped at 12 symbols")
...
class OrderBookState:
    ...
    def apply(self, message_type: str, data: dict[str, Any]) -> dict[str, float]:
        if message_type == "snapshot" or int(data.get("u") or 0) == 1:
            ...
            self._load_snapshot(self.bids, data.get("b") or [])
            self._load_snapshot(self.asks, data.get("a") or [])
        else:
            changes = self._apply_delta(data)
        if data.get("u") is not None:
            self.update_id = int(data["u"])
        if data.get("seq") is not None:
            self.cross_seq = int(data["seq"])
        return changes

    @staticmethod
    def _load_snapshot(book: dict[float, float], levels: list[list[str]]) -> None:
    ...
    def _apply_delta(self, data: dict[str, Any]) -> dict[str, float]:
        ...
        for side, levels, book in (
            ...
            for raw_price, raw_size in levels:
                ...
                delta = size - previous
                if delta > 0:
                    changes[f"{side}_added"] += price * delta
                elif delta < 0:
                    changes[f"{side}_removed"] += price * -delta
                if size <= 0:
                    book.pop(price, None)
                else:
                    book[price] = size
...
def depth_metrics(book: OrderBookState) -> dict[str, float | None]:
    ...
    def qdepth(levels: list[tuple[float, float]], count: int) -> float:
...
class ResearchBucket:
    ...
    def add_trade(self, trade: dict[str, Any]) -> None:
        ...
        if trade.get("T") is not None:
            self.last_trade_ms = max(self.last_trade_ms or 0, int(trade["T"]))
    ...
    def add_book(
        ...
        *, cts_ms: int | None,
        ...
        if cts_ms:
            self.last_book_ms = max(self.last_book_ms or 0, cts_ms)
        self.book_update_id = update_id
        self.cross_seq = cross_seq

    def db_values(self, source: str) -> tuple[Any, ...]:
        total_base = self.taker_buy_base + self.taker_sell_base
        trade_vwap = self.total_quote_volume / total_base if total_base > 0 else None
        m = self.book_metrics
        return (
            self.symbol, _utc_from_ms(self.start_ms), self.bucket_seconds, source,
            self.trade_count, self.block_trade_count, self.rpi_trade_count,
            self.taker_buy_base, self.taker_sell_base, self.taker_buy_quote, self.taker_sell_quote,
            self.taker_buy_quote - self.taker_sell_quote, self.total_quote_volume, trade_vwap,
            ...
            self.book_message_count, _utc_from_ms(self.last_trade_ms), _utc_from_ms(self.last_book_ms),
...
class MicrostructureRecorder:
    ...
    def _bucket(self, symbol: str, timestamp_ms: int) -> ResearchBucket:
        start_ms = _bucket_start_ms(timestamp_ms, self.config.bucket_seconds)
        key = (symbol, start_ms)
        if key not in self.buckets:
            self.buckets[key] = ResearchBucket(symbol, start_ms, self.config.bucket_seconds)
        ...
        while not self._stop.is_set():
            try:
                async with websockets.connect(
                    self.config.ws_url, ping_interval=None, close_timeout=5, max_queue=4096
                ) as ws:
    ...
    async def _subscribe(self, ws: Any) -> None:
        for batch in subscription_topics(self.config):
    ...
    def _handle_message(self, raw: str | bytes) -> None:
        ...
        if not topic:
            return
        self.runtime.last_message_at = datetime.now(timezone.utc).isoformat()
    ...
    def _handle_book(self, message: dict[str, Any]) -> None:
        ...
        self._bucket(symbol, cts_ms).add_book(
            depth_metrics(book), changes, cts_ms=cts_ms,
            update_id=book.update_id, cross_seq=book.cross_seq, ready=book.ready,
        )
    ...
    async def _flush_closed(self, *, force: bool) -> None:
        if self._db is None:
            return
        current_start = _bucket_start_ms(int(time.time() * 1000), self.config.bucket_seconds)
