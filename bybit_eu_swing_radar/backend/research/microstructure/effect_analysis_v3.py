...
# The production journal stores the cost-adjusted R outcome in `net_r`.
# Repository methodology explicitly states that Net R subtracts the configured
# round-trip cost assumption, so no synthetic alias is introduced here.
...
def validate_effect_preregistration(spec: dict[str, Any]) -> tuple[bool, str]:
    ...
    if spec.get("primary_outcome_semantics") != "cost-adjusted net R from day_trade_signal_journal.net_r":
