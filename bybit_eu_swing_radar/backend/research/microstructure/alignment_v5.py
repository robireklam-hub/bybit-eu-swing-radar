"""Preregistered v0.7.7 forward microstructure cohort.

V1-v4 remain immutable historical research cohorts. This v5 study reuses the
frozen label-blind feature definitions, hypotheses, windows and sample gates
while isolating all observations to live day strategy v0.7.7 and to a
...
from research.microstructure import alignment_v4 as v4
...
PARENT_SPEC_VERSION = v4.SPEC_VERSION
FEATURE_DEFINITION_SOURCE = v1.SPEC_VERSION
PREREGISTERED_STRATEGY_VERSION = "0.7.7"

# Day v0.7.7 merged in PR #466 as 04116db7... . Exact current production,
# recorder health/readiness and the still-label-blind historical cohorts were
# verified by NEVER-MERGE PR #468. Start strictly after that verification.
PRODUCTION_VERIFIED_BY = datetime(2026, 8, 22, 5, 54, 26, tzinfo=timezone.utc)
COHORT_START_AT = datetime(2026, 8, 22, 5, 55, 0, tzinfo=timezone.utc)
PRODUCTION_ACTIVATION_EVIDENCE = {
    "strategy_merge_sha": "04116db76f92dc1738071c9e5d774b55b69a1fc2",
...
def build_feature_rows(
    ...
    if contaminated:
        raise ValueError("v5 alignment strategy contamination: " + ",".join(contaminated))
...
async def load_feature_rows(
    ...
    """Load only v0.7.7 signals at/after the preregistered v5 forward start."""
    if not database_url:
