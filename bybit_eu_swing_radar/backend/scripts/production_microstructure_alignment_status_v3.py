#!/usr/bin/env python3
"""Read-only production check for the preregistered v0.7.5 microstructure cohort."""
...
PARENT_SPEC_VERSION = "microstructure-forward-alignment-v2"
...
def fetch_json(url: str, api_key: str, timeout: float = 15.0) -> dict[str, Any]:
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "bybit-eu-microstructure-alignment-status-v3/1",
            "X-Radar-Key": api_key,
        },
    )
...
def validate_alignment_status(payload: dict[str, Any]) -> tuple[bool, str]:
    ...
    if not isinstance(evidence, dict) or evidence.get("exact_production_verifier_pr") != 275:
...
def main() -> int:
    ...
    if not base_url or not api_key or not expected_sha:
        print("FAIL required alignment v3 status configuration is missing")
        return 1
    try:
        ...
        if version.get("commit_sha") != expected_sha:
            ...
            return 1
        payload = fetch_json(
            f"{base_url}/v1/research/microstructure/alignment-status-v3",
            api_key,
        )
    except HTTPError as exc:
        print(f"FAIL phase=alignment_v3 http_status={exc.code}")
        return 1
    except (URLError, TimeoutError, OSError, ValueError, RuntimeError) as exc:
        print(f"FAIL phase=alignment_v3 error_type={type(exc).__name__}")
        return 1
    ...
    print("ALIGNMENT_V3_STATUS=" + json.dumps(safe, sort_keys=True))
    if not ok:
        print(f"FAIL phase=alignment_v3 reason={reason}")
        return 1
    print("MICROSTRUCTURE V3 ALIGNMENT SAMPLE STATUS VERIFIED.")
    return 0
