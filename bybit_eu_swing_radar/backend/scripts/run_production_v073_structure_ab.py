"""Dispatch and monitor the production v0.7.3 pivot-structure A/B research replay."""
...
BATCH_COUNT = max(1, min(int(os.environ.get("STRUCTURE_AB_BATCH_COUNT", "1")), 15))
POLL_SECONDS = 10
ADVANCE_TIMEOUT_SECONDS = 20 * 60

STATUS_PATH = "/v1/day-trade/backtest/structure-ab/v073/status"
RUN_PATH = "/v1/day-trade/backtest/structure-ab/v073/run-batch"
REPORT_PATH = "/v1/day-trade/backtest/structure-ab/v073/report"
...
def request_json(path: str, *, method: str = "GET") -> dict:
    ...
    try:
        ...
        raise RuntimeError(
            f"Structure A/B API HTTP {exc.code} {path}: {body[:1200]}"
        ) from exc
...
def main() -> int:
    ...
    for batch_index in range(1, BATCH_COUNT + 1):
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise RuntimeError(
                    f"Timed out waiting for structure A/B batch {batch_index} to advance"
                )
    ...
    if job.get("status") in {"COMPLETED", "PARTIAL"}:
        ...
        print("V073_STRUCTURE_AB_REPORT=" + json.dumps(report, sort_keys=True, default=str))
        decision = ((report.get("go_criteria") or {}).get("decision"))
        a = (((report.get("models") or {}).get("A_RANGE6") or {}).get("overall") or {})
        b = (((report.get("models") or {}).get("B_PIVOT2L2R") or {}).get("overall") or {})
        print(
            "AB_SUMMARY "
            f"decision={decision} "
        ...
        blocks = (((report.get("models") or {}).get("B_PIVOT2L2R") or {}).get("blocks_30d") or [])
        for block in blocks:
            print(
                "B_BLOCK "
