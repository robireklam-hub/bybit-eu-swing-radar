#!/usr/bin/env python3
"""Exact-SHA production smoke for prospective Signal-Time Freeze v2."""
...
def _request(base: str, path: str, key: str, *, method: str = "GET", timeout: float = 45.0) -> dict[str, Any]:
    headers = {"Accept": "application/json", "User-Agent": "signal-freeze-v2-production-smoke/1"}
    if key:
...
def main() -> int:
    ...
    spec = _request(base, "/v1/research/signal-context-freeze-v2/spec", key, timeout=20)
    if spec.get("version") != "signal-context-freeze-v2":
        print("FAIL unexpected signal freeze v2 spec")
        return 1
    if (spec.get("source_layers") or {}).get("cross_layer_context") != "cross-layer-context-shadow-v2":
        print("FAIL signal freeze v2 is not bound to Cross-Layer v2")
        return 1
    if spec.get("historical_backfill_allowed") is not False or spec.get("v1_preserved") is not True:
    ...
    before = _request(base, "/v1/research/signal-context-freeze-v2/status", key, timeout=25)
    capture = _request(base, "/v1/research/signal-context-freeze-v2/capture", key, method="POST", timeout=55)
    after = _request(base, "/v1/research/signal-context-freeze-v2/status", key, timeout=25)
    ...
    safe = {
        ...
        "pre_v2_journal_signals_excluded": after.get("pre_v2_journal_signals_excluded"),
    ...
    print("SIGNAL_CONTEXT_FREEZE_V2=" + json.dumps(safe, sort_keys=True))

    if capture.get("source_commit_sha") != expected_sha:
        print("FAIL signal freeze v2 capture not on exact SHA")
        return 1
    ...
    if first_opened is not None and first_opened < start:
        print("FAIL pre-v2 signal leaked into v2 freeze cohort")
        return 1
    ...
    if int(after.get("frozen_signal_count") or 0) > int(after.get("prospective_journal_signal_count") or 0):
        print("FAIL v2 frozen count exceeds prospective journal population")
        return 1
