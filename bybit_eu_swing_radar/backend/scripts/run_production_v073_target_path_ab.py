"""Dispatch and monitor the production v0.7.3 target-path A/B/C research replay."""
...
def request_json(path: str, *, method: str = "GET") -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Target-path A/B/C API HTTP {exc.code} {path}: {body[:1200]}") from exc
...
def _metrics(report: dict, model: str) -> dict:
    return (((report.get("models") or {}).get(model) or {}).get("overall") or {})
...
def main() -> int:
    ...
    for batch_index in range(1, BATCH_COUNT + 1):
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise RuntimeError(f"Timed out waiting for target-path A/B/C batch {batch_index} to advance")
    ...
    if job.get("status") in {"COMPLETED", "PARTIAL"}:
        ...
        decision = ((report.get("go_criteria") or {}).get("decision"))
        current = _metrics(report, "A_CURRENT")
        fresh = _metrics(report, "B_FRESH_15M_CLOSE")
        ignore = _metrics(report, "C_IGNORE")
        print(
            "TARGET_PATH_SUMMARY "
            f"decision={decision} "
            f"A_n={current.get('sample_size')} A_avg_r={current.get('average_net_r')} A_pf={current.get('profit_factor')} "
            f"B_n={fresh.get('sample_size')} B_avg_r={fresh.get('average_net_r')} B_pf={fresh.get('profit_factor')} "
            f"C_n={ignore.get('sample_size')} C_avg_r={ignore.get('average_net_r')} C_pf={ignore.get('profit_factor')}"
        )
        for block in (((report.get("models") or {}).get("B_FRESH_15M_CLOSE") or {}).get("blocks_30d") or []):
            print(
                "B_BLOCK "
