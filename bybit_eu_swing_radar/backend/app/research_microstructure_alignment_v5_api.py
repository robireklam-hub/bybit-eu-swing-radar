"""Research-only production status for the preregistered v0.7.7 microstructure cohort."""
...
from research.microstructure.alignment_v5 import (
    COHORT_START_AT,
...
async def _load_v5_journal_signal_counts(
    database_url: str,
    symbols: Iterable[str],
    since: datetime,
    until: datetime,
) -> dict[str, int]:
    """Count only v0.7.7 signals inside the independently preregistered v5 cohort."""
    if not database_url:
...
def attach_microstructure_alignment_v5_research(
    app: FastAPI,
    require_api_key: Callable[..., Any],
) -> None:
    @app.get(
        "/v1/research/microstructure/alignment-status-v5",
        dependencies=[Depends(require_api_key)],
    )
    async def alignment_status_v5() -> dict[str, Any]:
        """Report label-blind v0.7.7 prospective sample readiness only."""
        try:
            ...
            if readiness_payload.get("ready_for_forward_feature_analysis") is not True:
                return build_alignment_v5_status(
                    readiness_payload,
                    [],
                    config.symbols,
                    empty_counts,
            ...
            features, journal_signal_counts = await asyncio.gather(
                ...
                _load_v5_journal_signal_counts(
                    config.database_url,
                    config.symbols,
                    since,
                    until,
            ...
            payload = build_alignment_v5_status(
                readiness_payload,
                features,
                config.symbols,
            ...
            logger.exception("microstructure v5 alignment status query failed")
            return {
                "research_only": True,
                "live_strategy_mutated": False,
                "label_blind": True,
