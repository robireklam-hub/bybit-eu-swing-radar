"""Research-only production status for the preregistered v0.7.5 microstructure cohort."""
...
from research.microstructure.alignment_v3 import (
    COHORT_START_AT,
...
async def _load_v3_journal_signal_counts(
    database_url: str,
    symbols: Iterable[str],
    since: datetime,
    until: datetime,
) -> dict[str, int]:
    """Count only v0.7.5 signals inside the independently preregistered v3 cohort."""
    if not database_url:
...
def build_alignment_v3_status(
    readiness_payload: Mapping[str, Any],
    features: Iterable[Mapping[str, Any]],
    symbols: Iterable[str],
    journal_signal_counts: Mapping[str, int],
) -> dict[str, Any]:
    """Combine frozen v3 data-quality, coverage and sample gates without outcomes."""
...
def attach_microstructure_alignment_v3_research(
    app: FastAPI,
    require_api_key: Callable[..., Any],
) -> None:
    @app.get(
        "/v1/research/microstructure/alignment-status-v3",
        dependencies=[Depends(require_api_key)],
    )
    async def alignment_status_v3() -> dict[str, Any]:
        """Report label-blind v0.7.5 prospective sample readiness only."""
        try:
            ...
            logger.exception("microstructure v3 alignment status query failed")
            return {
                "research_only": True,
                "live_strategy_mutated": False,
                "label_blind": True,
