"""Hidden API for prospective-only Signal-Time Context Freeze v2."""
...
async def _prospective_start_at(connection: asyncpg.Connection) -> datetime | None:
    try:
        return await connection.fetchval(
            "SELECT MIN(captured_at) FROM research_cross_layer_context_snapshots WHERE spec_version=$1",
            CROSS_LAYER_SPEC_VERSION,
        )
...
async def status_payload() -> dict[str, Any]:
    ...
    try:
        ...
        if start_at is None:
            journal_total = 0
            pre_v2_count = await connection.fetchval(
    ...
    return {
        ...
        "live_strategy_mutated": False,
        "promotion_allowed": False,
        "pre_v2_journal_signals_excluded": int(pre_v2_count or 0),
...
def attach_signal_context_freeze_v2_research(app: FastAPI, require_api_key: Callable[..., Any]) -> None:
    @app.get("/v1/research/signal-context-freeze-v2/spec", dependencies=[Depends(require_api_key)], include_in_schema=False)
    async def signal_context_freeze_v2_spec() -> dict[str, Any]:
        return spec()

    @app.post("/v1/research/signal-context-freeze-v2/capture", dependencies=[Depends(require_api_key)], include_in_schema=False)
    ...
    @app.get("/v1/research/signal-context-freeze-v2/status", dependencies=[Depends(require_api_key)], include_in_schema=False)
