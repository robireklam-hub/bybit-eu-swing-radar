"""FastAPI attachment for the fixed v0.7.3 target-path A/B research replay."""
...
async def _run_background() -> None:
    ...
    try:
        ...
        logger.info("v0.7.3 target-path A/B batch finished: %s", result)
    except Exception:
        logger.exception("v0.7.3 target-path A/B batch failed")
    finally:
...
async def target_path_ab_report_payload() -> dict[str, Any]:
    ...
    try:
        try:
            ...
            raise HTTPException(
                status_code=404,
                detail="v0.7.3 target-path A/B tables are not initialized",
            ) from exc
        if not job_raw:
            raise HTTPException(
                status_code=404,
                detail="v0.7.3 target-path A/B job not found",
            )
        job = dict(job_raw)
        if job.get("status") not in {"COMPLETED", "PARTIAL"}:
            raise HTTPException(
                status_code=409,
                detail=f"target-path A/B job must be terminal, got {job.get('status')}",
            )
        rows = await conn.fetch(
            """
...
def attach_v073_target_path_ab_routes(
    ...
    async def run_batch() -> dict[str, Any]:
        ...
        if _target_path_ab_task is not None and not _target_path_ab_task.done():
            raise HTTPException(
                status_code=409,
                detail="v0.7.3 target-path A/B batch already running",
            )
        _target_path_ab_task = asyncio.create_task(
            _run_background(),
            name="v073-target-path-active-barrier-ab-batch",
        )
        return {
            "accepted": True,
            "strategy_version": STRATEGY_VERSION,
