...
async def fetch_analysis_bars(api: BybitAPI, instrument: Instrument, semaphore: asyncio.Semaphore) -> tuple[Instrument, list[Bar], list[Bar], list[Bar]] | None:
    try:
...
async def fetch_momentum_bars(api: BybitAPI, instrument: Instrument, semaphore: asyncio.Semaphore) -> tuple[Instrument, list[Bar], list[Bar]] | None:
    try:
        async with semaphore:
        ...
        if len(bars_5m) < 18 or len(bars_15m) < 12:
            print(f"WARN {instrument.symbol}: insufficient momentum history (5m={len(bars_5m)}, 15m={len(bars_15m)})", file=sys.stderr)
            return None
...
def analyze_momentum(instrument: Instrument, bars_5m: list[Bar], bars_15m: list[Bar]) -> MomentumAnalysis:
    ...
    if atr_15m <= 0:
        return MomentumAnalysis(
            instrument=instrument, bars_5m=bars_5m, bars_15m=bars_15m,
            score=0.0, side="neutral", return_15m_pct=bars_return_pct(bars_5m, 3),
            return_1h_pct=bars_return_pct(bars_15m, 4), return_4h_pct=bars_return_pct(bars_15m, 16),
            previous_1h_return_pct=0.0, acceleration_pct=0.0, volume_ratio_5m=0.0,
            turnover_acceleration_1h=0.0, atr_15m=0.0, extension_atr=0.0,
            breakout_price=current, breakout_confirmed=False, distance_to_breakout_atr=0.0,
            chase_risk=False, stage="NO_ACTIVITY",
    ...
    prior_hours = [sum(bar.turnover for bar in bars_15m[start:start + 4]) for start in range(max(0, len(bars_15m) - 28), len(bars_15m) - 4, 4) if len(bars_15m[start:start + 4]) == 4]
    ...
    alignment = ((return_15m >= 0 and return_1h >= 0 and return_4h >= 0) or (return_15m <= 0 and return_1h <= 0 and return_4h <= 0))
    ...
    score = clamp(0.20 * price_score + 0.20 * volume_score + 0.15 * turnover_score + 0.15 * acceleration_score + 0.15 * breakout_score + 0.15 * daily_move_score + alignment_bonus + extreme_move_bonus)
    chase_risk = bool(abs(instrument.price_change_24h_pct) >= 25.0 or abs(return_4h) >= 20.0 or extension_atr >= 3.5)
    if chase_risk:
        ...
        stage = "LOW_SIGNAL"
    return MomentumAnalysis(instrument=instrument, bars_5m=bars_5m, bars_15m=bars_15m, score=score, side=side, return_15m_pct=return_15m, return_1h_pct=return_1h, return_4h_pct=return_4h, previous_1h_return_pct=previous_1h, acceleration_pct=acceleration, volume_ratio_5m=volume_ratio_5m, turnover_acceleration_1h=turnover_acceleration, atr_15m=atr_15m, extension_atr=extension_atr, breakout_price=breakout_price, breakout_confirmed=breakout_confirmed, distance_to_breakout_atr=distance_to_breakout_atr, chase_risk=chase_risk, stage=stage, missing_data=[])
...
def build_momentum_item(momentum: MomentumAnalysis, now: datetime) -> dict[str, Any]:
    ...
    state = "NO_TRADE" if momentum.stage == "NO_ACTIVITY" else "TRIGGERED" if momentum.breakout_confirmed and momentum.score >= 70 else "WATCH" if momentum.score >= MOMENTUM_MIN_SCORE else "NO_TRADE"
    reasons = [f"{momentum.return_1h_pct:+.2f}% 1H move", f"{momentum.volume_ratio_5m:.2f}x 5m relative volume", f"{momentum.turnover_acceleration_1h:.2f}x hourly turnover acceleration"]
    ...
    return {
        "symbol": instrument.symbol, "base_asset": instrument.base, "quote_asset": "USDC", "side": side,
        "state": state, "stage": momentum.stage, "momentum_score": round(momentum.score, 2), "last_price": instrument.last_price,
        "price_change_24h_pct": round(instrument.price_change_24h_pct, 3), "return_15m_pct": round(momentum.return_15m_pct, 3),
        "return_1h_pct": round(momentum.return_1h_pct, 3), "return_4h_pct": round(momentum.return_4h_pct, 3),
        "acceleration_pct": round(momentum.acceleration_pct, 3), "volume_ratio_5m": round(momentum.volume_ratio_5m, 3),
        "turnover_acceleration_1h": round(momentum.turnover_acceleration_1h, 3), "extension_atr_15m": round(momentum.extension_atr, 3),
        "breakout_confirmed": momentum.breakout_confirmed, "chase_risk": momentum.chase_risk, "tradeable": instrument.tradeable,
        "execution_status": execution_status, "turnover_24h_usdc": round(instrument.turnover_24h, 2), "spread_bps": round(instrument.spread_bps, 3),
        "trigger": {"timeframe": "15m", "condition": condition, "price": None if side == "neutral" else round_to_tick(momentum.breakout_price, instrument.tick_size), "requires_close": True, "volume_confirmation": None if side == "neutral" else "Prefer >=2.0x recent 5m relative volume and rising hourly turnover"},
        "invalidation_price": None if invalidation_price is None else round_to_tick(invalidation_price, instrument.tick_size),
        "bullish_scenario": bullish, "bearish_scenario": bearish, "why_now": reasons,
...
def rank_momentum(momentums: list[MomentumAnalysis], now: datetime, cache_min_score: float = 0.0) -> list[dict[str, Any]]:
    ranked = sorted([item for item in momentums if item.score >= cache_min_score], key=lambda item: (item.stage == "BREAKOUT_CONFIRMED", item.stage == "EARLY_ACCELERATION", item.stage == "MOMENTUM_WATCH", item.score, abs(item.instrument.price_change_24h_pct), item.instrument.turnover_24h), reverse=True)
    return [build_momentum_item(item, now) for item in ranked[:MOMENTUM_OUTPUT_LIMIT]]


def analyze_market(instrument: Instrument, bars_1h: list[Bar], bars_4h: list[Bar], bars_1d: list[Bar], btc_return_4h: float) -> Analysis:
    ...
    expansion_score = 0.25 * compression_score + 0.25 * bb_compression_score + 0.20 * boundary_score + 0.15 * volume_score + 0.15 * maturity_score
    direction = 0.0
    ...
    return Analysis(instrument=instrument, bars_1h=bars_1h, bars_4h=bars_4h, bars_1d=bars_1d, atr_4h=atr_4h, ema20_4h=ema20_4h, ema50_4h=ema50_4h, ema20_1d=ema20_1d, ema50_1d=ema50_1d, range_high=range_high, range_low=range_low, recent_high=recent_high, recent_low=recent_low, volume_ratio=volume_ratio, bb_width_percentile=bb_percentile, atr_ratio=atr_ratio, expansion_score=clamp(expansion_score), direction_score=direction, quality_score=clamp(quality_score), relative_strength_4h=relative_strength, structure_4h=structure_label(current, ema20_4h, ema50_4h), structure_1d=structure_label(close_1d, ema20_1d, ema50_1d), derivatives={}, missing_data=[])


def select_coinalyze_markets(markets: list[dict[str, Any]], bases: list[str]) -> dict[str, dict[str, Any]]:
...
async def enrich_coinalyze(
    ...
    *,
    ...
    """Attach Coinalyze context while tolerating partial endpoint failures.

    ``mutate_scores`` defaults to True only for backward compatibility with the
    existing day-worker import path. The swing worker calls this with False so
    derivatives can never change swing strict eligibility.
    """
    ...
    market_map = select_coinalyze_markets(
        markets, [item.instrument.base for item in selected_analyses]
    )
    symbols = [
    ...
    async def optional(label: str, awaitable: Any) -> Any:
        try:
            ...
            endpoint_errors.append(f"{label}: {type(exc).__name__}: {exc}")
            return []

    # Sequential batches intentionally avoid the previous four-request burst.
    # With the default 9-symbol target these consume 36 symbol-calls, below the
    # documented 40 symbol-call/minute limit, while preserving Retry-After
    # handling in CoinalyzeAPI.get.
    ...
    oi_map = {item["symbol"]: item for item in current_oi if isinstance(item, dict) and item.get("symbol")}
    funding_map = {item["symbol"]: item for item in current_funding if isinstance(item, dict) and item.get("symbol")}
    oi_hist_map = {item["symbol"]: item.get("history", []) for item in oi_history if isinstance(item, dict) and item.get("symbol")}
    liq_map = {item["symbol"]: item.get("history", []) for item in liquidation_history if isinstance(item, dict) and item.get("symbol")}
    ...
    for analysis in analyses:
        ...
        if not (has_current_oi or has_funding or has_oi_history or has_liquidations):
            analysis.missing_data.append("Coinalyze data unavailable for matched market")
            continue
        ...
        liq_rows = liq_map.get(symbol, [])[-6:] if has_liquidations else []
        long_liq = sum(safe_float(row.get("l")) for row in liq_rows) if has_liquidations else None
        short_liq = sum(safe_float(row.get("s")) for row in liq_rows) if has_liquidations else None
        funding = safe_float(funding_map[symbol].get("value"), 0.0) if has_funding else None
        current_oi_value = safe_float(oi_map[symbol].get("value"), 0.0) if has_current_oi else None
        ...
        if oi_change_24h_pct is not None:
            ...
            if oi_change_24h_pct > 0:
                ...
                expansion_delta += clamp(abs(oi_change_24h_pct) * 0.5, 0.0, 8.0)
            else:
                quality_delta -= clamp(abs(oi_change_24h_pct) * 0.3, 0.0, 5.0)
        ...
        if has_liquidations:
            liquidation_ratio = ((long_liq or 0.0) + (short_liq or 0.0)) / max(analysis.instrument.turnover_24h, 1.0)
            expansion_delta += clamp(liquidation_ratio * 300.0, 0.0, 5.0)
        ...
        if endpoint_errors:
            analysis.missing_data.append("Coinalyze derivatives context partial")

        if mutate_scores:
            analysis.expansion_score = clamp(analysis.expansion_score + expansion_delta)
            analysis.direction_score = clamp(analysis.direction_score + direction_delta, -100.0, 100.0)
            analysis.quality_score = clamp(analysis.quality_score + quality_delta)

    error_parts = list(endpoint_errors)
    if len(symbols) < targeted_count:
        error_parts.append(f"market-match coverage {len(symbols)}/{targeted_count}")
    if enriched_count < targeted_count:
        error_parts.append(f"enrichment coverage {enriched_count}/{targeted_count}")
    ...
    try:
        ...
        for analysis in analyses:
            ...
            if margin_pair and not analysis.shortable:
                analysis.missing_data.append("USDC spot-margin base coin is not borrowable in public Bybit margin data")
        return True, None
    except Exception as exc:
        for analysis in analyses:
...
def build_setup(analysis: Analysis, side: str, now: datetime) -> dict[str, Any] | None:
    ...
    if side == "long":
        ...
        thesis = [f"4H structure: {analysis.structure_4h}", f"1D structure: {analysis.structure_1d}", f"BTC-relative 20x4H strength: {analysis.relative_strength_4h:+.2f}%", f"4H volume ratio: {analysis.volume_ratio:.2f}x"]
        ...
        thesis = [f"4H structure: {analysis.structure_4h}", f"1D structure: {analysis.structure_1d}", f"BTC-relative 20x4H strength: {analysis.relative_strength_4h:+.2f}%", f"Public Bybit max borrowing amount: {analysis.max_borrowing_amount:g} {instrument.base}"]
    ...
    risks = ["BTC regime reversal can invalidate altcoin relative-strength signals.", "Coinalyze derivatives context may represent another venue or an aggregate market."]
    if side == "short":
    ...
    metrics = {"turnover_24h_usdc": round(instrument.turnover_24h, 2), "spread_bps": round(instrument.spread_bps, 3), "atr_4h": atr_value, "atr_percent_of_price": round(atr_value / last_price * 100.0, 3), "atr_ratio_to_50bar_average": round(analysis.atr_ratio, 3), "bb_width_percentile": round(analysis.bb_width_percentile, 2), "volume_ratio_4h": round(analysis.volume_ratio, 3), "relative_strength_vs_btc_20x4h_pct": round(analysis.relative_strength_4h, 3), "margin_trading_flag": instrument.margin_trading, "max_borrowing_amount_base": analysis.max_borrowing_amount, "derivatives": analysis.derivatives}
    execution_modes = ["spot_usdc"] if side == "long" else ["spot_margin_short_usdc"]
    return {"symbol": instrument.symbol, "base_asset": instrument.base, "quote_asset": "USDC", "side": side, "state": state, "grade": grade, "confidence": confidence, "last_price": last_price, "shortable": analysis.shortable, "execution_modes": execution_modes, "setup_type": "20-bar range expansion / structure continuation", "thesis": thesis, "expansion_score": round(analysis.expansion_score, 2), "direction_score": round(analysis.direction_score, 2), "quality_score": round(analysis.quality_score, 2), "setup_score": round(score, 2), "trigger": {"timeframe": "4H", "condition": condition, "price": round_to_tick(trigger_price, instrument.tick_size), "requires_close": True, "volume_confirmation": "Prefer >=1.2x the prior 20-bar average 4H volume"}, "entry_zone": {"low": round_to_tick(entry_low, instrument.tick_size), "high": round_to_tick(entry_high, instrument.tick_size)}, "stop": round_to_tick(stop, instrument.tick_size), "invalidation": invalidation, "targets": [round_to_tick(value, instrument.tick_size) for value in targets], "expected_rr": 2.5, "expected_holding_days": "2-10", "metrics": metrics, "bullish_scenario": bullish, "bearish_scenario": bearish, "weakest_point": "The setup is heuristic and remains unconfirmed until the 4H trigger closes with volume.", "risks": risks, "data_quality": data_quality, "missing_data": sorted(set(analysis.missing_data)), "data_as_of": now.isoformat()}
...
def build_watch_setup(analysis: Analysis, now: datetime) -> dict[str, Any]:
    ...
    data_quality = "GOOD" if analysis.derivatives and not analysis.missing_data else "PARTIAL"
    execution_modes = ["spot_usdc_watch_only"] if side != "short" else ["spot_margin_short_usdc_watch_only"]
    return {"symbol": instrument.symbol, "base_asset": instrument.base, "quote_asset": "USDC", "side": side, "state": "WATCH" if score >= 60 else "NO_TRADE", "grade": "WATCH" if score >= 60 else "NO_TRADE", "confidence": "MEDIUM" if score >= 70 and data_quality == "GOOD" else "LOW", "last_price": last_price, "shortable": analysis.shortable, "execution_modes": execution_modes, "setup_type": "Extended discovery watch / 20-bar range expansion", "thesis": [f"4H structure: {analysis.structure_4h}", f"1D structure: {analysis.structure_1d}", f"BTC-relative 20x4H strength: {analysis.relative_strength_4h:+.2f}%", "WATCH_ONLY: technical interest is separated from execution quality."], "expansion_score": round(analysis.expansion_score, 2), "direction_score": round(analysis.direction_score, 2), "quality_score": round(analysis.quality_score, 2), "setup_score": round(score, 2), "trigger": {"timeframe": "4H", "condition": condition, "price": round_to_tick(trigger_price, instrument.tick_size), "requires_close": True, "volume_confirmation": "Prefer >=1.2x the prior 20-bar average 4H volume"}, "entry_zone": {"low": round_to_tick(entry_low, instrument.tick_size), "high": round_to_tick(entry_high, instrument.tick_size)}, "stop": round_to_tick(stop, instrument.tick_size), "invalidation": invalidation, "targets": [round_to_tick(value, instrument.tick_size) for value in targets], "expected_rr": 2.5, "expected_holding_days": "2-10", "metrics": {"execution_status": "LIQUIDITY_BLOCKED" if liquidity_blocked else "WATCH_ONLY", "liquidity_reasons": reasons, "turnover_24h_usdc": round(instrument.turnover_24h, 2), "spread_bps": round(instrument.spread_bps, 3), "tradeable": instrument.tradeable, "discovery_source": instrument.discovery_source, "coinalyze_enriched": bool(analysis.derivatives), "margin_trading_flag": instrument.margin_trading, "max_borrowing_amount_base": analysis.max_borrowing_amount, "atr_4h": atr_value, "volume_ratio_4h": round(analysis.volume_ratio, 3), "relative_strength_vs_btc_20x4h_pct": round(analysis.relative_strength_4h, 3), "derivatives": analysis.derivatives}, "bullish_scenario": bullish_scenario, "bearish_scenario": bearish_scenario, "weakest_point": "; ".join(reasons) if reasons else "Trigger is not yet confirmed.", "risks": ["WATCH_ONLY is not an execution signal.", "Low USDC turnover or wide spread can cause material slippage.", "Coinalyze derivatives context may be aggregated or from another venue."], "data_quality": data_quality, "missing_data": sorted(set(analysis.missing_data)), "data_as_of": now.isoformat()}
...
def rank_watchlist(analyses: list[Analysis], now: datetime, excluded_symbols: set[str]) -> list[dict[str, Any]]:
    ...
    items.sort(key=lambda item: (1 if item["symbol"] in DISCOVERY_SYMBOLS else 0, item["setup_score"]), reverse=True)
    return items
...
def build_market_regime(analyses: list[Analysis], now: datetime, coinalyze_ok: bool, borrow_ok: bool) -> dict[str, Any]:
    ...
    if target_count > 0 and enriched_count >= target_count and coinalyze_ok:
        coinalyze_quality = "GOOD"
    elif enriched_count > 0:
        coinalyze_quality = "PARTIAL"
    else:
        coinalyze_quality = "DEGRADED"
    quality = "GOOD" if len(analyses) >= 10 and coinalyze_quality == "GOOD" and borrow_ok else "PARTIAL"
    notes = [
    ...
    if not coinalyze_ok:
        notes.append("Coinalyze enrichment is partial or unavailable; available endpoint data is retained.")
    if not borrow_ok:
        notes.append("Public Bybit Spot Margin borrowability data is unavailable; executable short list is suppressed.")
    return {"data_as_of": now.isoformat(), "data_quality": quality, "btc_regime": btc_regime, "btc_structure_1d": structure_1d, "btc_structure_4h": structure_4h, "alt_breadth": round(alt_breadth, 2), "volatility_regime": volatility, "preferred_side": preferred, "source_quality": {"bybit_market_data": "GOOD" if analyses else "DEGRADED", "bybit_spot_margin": "GOOD" if borrow_ok else "PARTIAL", "coinalyze_derivatives": coinalyze_quality}, "notes": notes}


async def upsert_cache(connection: asyncpg.Connection, key: str, payload: dict[str, Any]) -> None:
    await connection.execute("""INSERT INTO radar_cache (cache_key, payload, updated_at) VALUES ($1, $2::jsonb, NOW()) ON CONFLICT (cache_key) DO UPDATE SET payload = EXCLUDED.payload, updated_at = NOW()""", key, json.dumps(payload, ensure_ascii=False))


async def persist_results(scan: dict[str, Any], regime: dict[str, Any], setups: list[dict[str, Any]], watchlist: list[dict[str, Any]], momentum_radar: dict[str, Any], status: dict[str, Any]) -> None:
    ...
    try:
        async with connection.transaction():
            ...
            await upsert_cache(connection, "watchlist", {"data_as_of": scan["data_as_of"], "items": watchlist})
            ...
            for setup in setups + watchlist:
                ...
                if existing is None or (setup_is_executable and not existing_is_executable) or (setup_is_executable == existing_is_executable and setup["setup_score"] > existing["setup_score"]):
...
async def run() -> None:
    ...
    async with httpx.AsyncClient(timeout=timeout, limits=limits, headers={"User-Agent": "Bybit-EU-Swing-Radar/0.4.0"}) as client:
        ...
        momentum_fetched = await asyncio.gather(*(fetch_momentum_bars(bybit, instrument, momentum_semaphore) for instrument in candidate_pool))
        momentum_by_symbol = {item[0].symbol: item for item in momentum_fetched if item is not None}
        for retry_pass in range(MOMENTUM_SYMBOL_RETRY_PASSES):
            missing_instruments = [instrument for instrument in candidate_pool if instrument.symbol not in momentum_by_symbol]
            ...
            retried = await asyncio.gather(*(fetch_momentum_bars(bybit, instrument, retry_semaphore) for instrument in missing_instruments))
            for item in retried:
                if item is not None:
                    momentum_by_symbol[item[0].symbol] = item
        momentum_valid = list(momentum_by_symbol.values())
        momentum_failed_symbols = sorted(instrument.symbol for instrument in candidate_pool if instrument.symbol not in momentum_by_symbol)
        ...
        momentum_promoted = [item.instrument for item in momentum_ranked_analyses if item.score >= MOMENTUM_MIN_SCORE][:MOMENTUM_DEEP_LIMIT]
        ...
        universe_stats.update({"analysis_universe_size": len(universe), "tradeable_universe_size": sum(1 for item in universe if item.tradeable), "liquidity_blocked_size": sum(1 for item in universe if not item.tradeable), "momentum_eligible_pairs": len(candidate_pool), "momentum_scanned_pairs": len(momentum_analyses), "momentum_failed_pairs": len(momentum_failed_symbols), "momentum_failed_symbols": momentum_failed_symbols, "momentum_calculation_failed_pairs": len(momentum_calculation_failures), "momentum_calculation_failures": momentum_calculation_failures, "momentum_no_activity_pairs": sum(1 for item in momentum_analyses if item.stage == "NO_ACTIVITY"), "momentum_candidates": sum(1 for item in momentum_analyses if item.score >= MOMENTUM_MIN_SCORE), "momentum_promoted_to_deep_scan": sum(1 for item in momentum_promoted if item.symbol in seen)})
        print("USDC analysis universe: " f"{len(universe)} symbols; tradeable={universe_stats['tradeable_universe_size']}; " f"liquidity_blocked={universe_stats['liquidity_blocked_size']}; " f"mandatory_found={universe_stats['mandatory_found']}; " f"raw_usdc_records={universe_stats['raw_usdc_instrument_records']}; " f"unique_usdc={universe_stats['unique_usdc_instruments']}; " f"duplicate_records={universe_stats['duplicate_instrument_records']}; " f"momentum_scanned={len(momentum_analyses)}; momentum_candidates={universe_stats['momentum_candidates']}")
        semaphore = asyncio.Semaphore(HTTP_CONCURRENCY)
        fetched = await asyncio.gather(*(fetch_analysis_bars(bybit, instrument, semaphore) for instrument in universe))
        ...
        coinalyze_ok, coinalyze_error = await enrich_coinalyze(analyses, coinalyze, mutate_scores=False)
        ...
        liquidity_blocked = [item for item in all_watchlist if item.get("metrics", {}).get("execution_status") == "LIQUIDITY_BLOCKED"]
        momentum_items = rank_momentum(momentum_analyses, now, cache_min_score=0.0)
        momentum_radar = {"data_as_of": now.isoformat(), "raw_usdc_instrument_records": universe_stats["raw_usdc_instrument_records"], "unique_usdc_instruments": universe_stats["unique_usdc_instruments"], "duplicate_instrument_records": universe_stats["duplicate_instrument_records"], "duplicate_symbols": universe_stats["duplicate_symbols"], "eligible_pairs": len(candidate_pool), "scanned_pairs": len(momentum_analyses), "failed_pairs": len(momentum_failed_symbols), "failed_symbols": momentum_failed_symbols, "calculation_failed_pairs": len(momentum_calculation_failures), "calculation_failures": momentum_calculation_failures, "no_activity_pairs": sum(1 for item in momentum_analyses if item.stage == "NO_ACTIVITY"), "minimum_score": 0.0, "promotion_minimum_score": MOMENTUM_MIN_SCORE, "items": momentum_items}
        regime = build_market_regime(analyses, now, coinalyze_ok, borrow_ok)
        enriched_count = sum(1 for item in analyses if item.derivatives)
        coinalyze_target_count = min(len(analyses), COINALYZE_ENRICH_LIMIT)
        data_quality = regime["data_quality"]
        coverage = {"raw_usdc_instrument_records": universe_stats["raw_usdc_instrument_records"], "unique_usdc_instruments": universe_stats["unique_usdc_instruments"], "duplicate_instrument_records": universe_stats["duplicate_instrument_records"], "duplicate_symbols": universe_stats["duplicate_symbols"], "analyzed_symbols": len(analyses), "coinalyze_enriched_symbols": enriched_count, "coinalyze_target_symbols": coinalyze_target_count, "coinalyze_enrichment_limit": COINALYZE_ENRICH_LIMIT, "borrowability_checked_symbols": len(analyses) if borrow_ok else 0, "symbols_missing_history": missing_history, "momentum_eligible_pairs": len(candidate_pool), "momentum_scanned_pairs": len(momentum_analyses), "momentum_failed_pairs": len(momentum_failed_symbols), "momentum_failed_symbols": momentum_failed_symbols, "momentum_calculation_failed_pairs": len(momentum_calculation_failures), "momentum_calculation_failures": momentum_calculation_failures, "momentum_no_activity_pairs": sum(1 for item in momentum_analyses if item.stage == "NO_ACTIVITY")}
        scan = {"data_as_of": now.isoformat(), "data_as_of_budapest": now.astimezone(BUDAPEST).isoformat(), "data_quality": data_quality, "market_regime": regime, "longs": long_setups[:10], "shorts": short_setups[:10], "extended_watchlist": watchlist, "liquidity_blocked": liquidity_blocked, "momentum_radar": momentum_items, "universe_stats": universe_stats, "coverage": coverage, "exclusions": exclusions[:100]}
        elapsed = (datetime.now(timezone.utc) - started).total_seconds()
        status = {"checked_at": now.isoformat(), "worker": {"status": "ok", "duration_seconds": round(elapsed, 2), **universe_stats, **coverage, "long_candidates": len(long_setups), "short_candidates": len(short_setups), "extended_watchlist_items": len(watchlist), "liquidity_blocked_items": len(liquidity_blocked), "momentum_eligible_pairs": len(candidate_pool), "momentum_scanned_pairs": len(momentum_analyses), "momentum_failed_pairs": len(momentum_failed_symbols), "momentum_failed_symbols": momentum_failed_symbols, "momentum_calculation_failed_pairs": len(momentum_calculation_failures), "momentum_calculation_failures": momentum_calculation_failures, "momentum_no_activity_pairs": sum(1 for item in momentum_analyses if item.stage == "NO_ACTIVITY"), "momentum_output_items": len(momentum_items)}, "sources": [{"source": "Bybit EU", "status": "ok", "data_as_of": now.isoformat(), "latency_seconds": 0, "missing_fields": []}, {"source": "Coinalyze", "status": "ok" if coinalyze_ok else "partial", "data_as_of": now.isoformat() if enriched_count > 0 else None, "latency_seconds": 0 if enriched_count > 0 else None, "coverage": f"{enriched_count}/{coinalyze_target_count} targeted ({len(analyses)} analyzed total)", "missing_fields": [] if coinalyze_error is None else [coinalyze_error]}, {"source": "Bybit EU Spot Margin", "status": "ok" if borrow_ok else "partial", "data_as_of": now.isoformat() if borrow_ok else None, "latency_seconds": 0 if borrow_ok else None, "missing_fields": [] if borrow_ok else [borrow_error or "public borrowability unavailable"]}]}
        await persist_results(scan, regime, long_setups + short_setups, watchlist, momentum_radar, status)
        print("Worker complete: " f"analyzed={len(analyses)}, tradeable={universe_stats['tradeable_universe_size']}, " f"watchlist={len(watchlist)}, liquidity_blocked={len(liquidity_blocked)}, " f"coinalyze={enriched_count}/{coinalyze_target_count} targeted, longs={len(long_setups)}, " f"shorts={len(short_setups)}, momentum={len(momentum_items)}, " f"momentum_coverage={len(momentum_analyses)}/{len(candidate_pool)}, " f"quality={data_quality}, duration={elapsed:.1f}s")
